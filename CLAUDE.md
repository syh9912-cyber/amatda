...
## 🚨 절대 규칙 — 코딩 원칙
> 이 앱은 정식 출시 상용 앱이다. 임시방편·우선 돌아가게만 고치는 방식은 절대 금지.
> **언제나 공식 문서 기반 정석 방식으로 구현한다.**

- 임시방편 금지: "일단 돌아가게" 수준의 코드 절대 금지
- 공식 문서 우선: React Native, Expo, Firebase 공식 패턴 확인 후 구현
- 구조적 해결: 증상만 패치하지 말고 근본 원인을 구조적으로 수정
- 예시:
  - 인증 → Expo Router Layout 게이트 패턴 (API 인터셉터에서 router 직접 호출 금지)
  - 상태 저장 → 원자적 저장 (setTokens + setUser 분리 호출 금지, setAuth 단일 호출)
  - 에러 처리 → 사용자에게 명확한 에러 + 재시도 제공 (조용히 삼키기 금지)
