...
## ⚠️ 출시 준비 모드 (2026-04-28 ~)

> **출시 목표: 2026-05-15** (조정 가능)
> 동결은 해제 — 베타 테스트에서 발견된 회귀 / UX 이슈가 많아 fix-and-polish 단계.
> 다만 다음은 여전히 사용자 명시 승인 필요(Rule of Two):
> - Firestore 스키마 변경
> - AI 파이프라인(`coaching.ts`, `prompt.builder.ts`) 순서 변경
> - 프롬프트 핵심 로직 변경
> - 인증 흐름 핵심 구조 변경
> - 보안 규칙(`firestore.rules`/`storage.rules`) 변경
> - 운영 환경 변수/시크릿 로드 방식 변경
>
> 일반 P0/P1 fix, UI 회귀 복구, UX 개선은 자유롭게 진행.
