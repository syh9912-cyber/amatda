...
const PREGNANT_FLAG_RULES: FlagRule[] = [
  ...
  { pattern: /(?:대량|다량|심한).*(?:출혈|피|하혈)|하혈.*(?:심|많|쏟|멈추지|계속)|피.*(?:많이|쏟|흐름)|大量.*出血|血が.*(?:たくさん|大量に)|血.*(?:很多|流不停|流個不停)/, label: '대량 출혈', urgency: 'emergency' },
  ...
  // ─── Urgent (24시간 내 진료) ───
  { pattern: /(?:출혈|피|하혈).*(?:나|묻|보|있|심)|하혈|질.*(?:출혈|피)|出血|性器.*(?:出血|血)|陰道.*(?:出血|流血)/, label: '질 출혈', urgency: 'urgent' },
