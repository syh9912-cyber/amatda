...
export default function LullabyScreen() {
  ...
  const playSound = useCallback(async (id: string) => {
    try {
      ...
      // Check if it's a built-in sound or user recording
  ...
  const currentSound = BUILT_IN_SOUNDS.find((s) => s.id === playing) ?? (
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerTitle}>자장가</Text>
        <Text style={styles.headerSub}>아이의 편안한 잠을 위해</Text>
      </View>
      ...
      {currentSound ? (
        ...
        <View style={styles.nowPlaying}>
          ...
          <Text style={styles.nowPlayingLabel}>소리를 선택하세요</Text>
      ...
      <View style={styles.cryRow}>
        <View style={styles.cryInfo}>
          <Text style={styles.cryLabel}>{'👂'} 울음 감지 자동 재생</Text>
